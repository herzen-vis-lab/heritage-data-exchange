...
# ─────────────────────────────────────────────────────────────────────────
# 7. Согласование классификаторов
# ─────────────────────────────────────────────────────────────────────────

rule classifier.versioning {
...
rule entity.identity_registrar {
  description: "Первый регистратор получает право управления локальной записью и идентификатором сущности (Work). Приоритет по времени может использоваться как правило определения первичного регистратора; оно применяется только к вопросам идентичности и права управления записью и не распространяется на истинность атрибутов"
...
rule conflict.resolution {
  description: "Каноническое значение выбирается явным правилом, предлагаемым как политика верификации по умолчанию (а не как найденное научно оптимальное число): (1) заявление становится canonical после N≥2 независимых подтверждений от разных узлов — порог конфигурируется участниками; (2) действует окно оспаривания — в течение периода любой узел может подать встречное заявление с обоснованием (ссылка на источник, экспертиза); (3) при отсутствии оспаривания заявление первого регистратора становится дефолтным — как fallback, а не единственное правило"
  threshold: N >= 2 independent confirmations (configurable, default policy)
